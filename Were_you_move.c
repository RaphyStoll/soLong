...
void move_is_possible(t_game *game, int target_x, int target_y)
{
	...
	else if (is_exit_close(game->map->map, target_x, target_y) && (game->map->collected != game->map->items))
    {
        moved_in_a_closed_exit(game, target_x, target_y);
        return ;
    }
	else if (is_exit_open(game->map->map, game->map->exit_isopen, target_x, target_y) && (game->map->collected == game->map->items))
    {
